...
import EditSubjectModal from '@/components/subjects/EditSubjectModal.vue';
...
const fetchData = async () => {
  ...
  try {
    const res:any = await $api.post("/subjects", {
...
const addDelete = async (id="0") => {
  ...
  try {
    const res:any = await $api.delete("/subjects/delete/"+id)
    const obj:any = res.data
    if (obj.status === 'success') {
...
<div class="dashwrap">
  ...
  <div class="section-hdr">
    ...
    <div class="section-hdr-right">
      <button class="btn btn-primary btn-sm" @click="openAddSubjectsModal">
        Add Subject
      </button>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="table-wrap">
        ...
        <table class="SubjectsTable">
          ...
          <tbody v-if="data_loading || filteredSubjects.length === 0">
            <tr>
              <td colspan="15">
          ...
          <tbody v-else>
            <tr v-for="(item, i) in filteredSubjects" :key="i">
              <td>
                {{i+1}}
              </td>
              <td>{{ item.name }}</td>
