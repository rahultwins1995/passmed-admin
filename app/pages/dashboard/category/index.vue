...
const addDelete = async (id="0") => {
  ...
  try {
    const res:any = await $api.delete("/categories/delete/"+id)
    const obj:any = res.data
    if (obj.status === 'success') {
  ...
  <div class="card">
    ...
    <div class="table-wrap">
        ...
        <table class="CategoryTable">
          ...
          <tbody v-if="data_loading || filteredCategory.length === 0">
            <tr>
              <td colspan="15">
          ...
          <tbody v-else>
            <tr v-for="(item, i) in filteredCategory" :key="i">
              <td>
                {{i+1}}
              </td>
              <td>{{ item.name }}</td>
              <!-- This column existed all along but was ALWAYS blank: Category::subject()
                   keyed off a `categories.category_id` column that never existed. Now
                   that categories.subject_id is real, it resolves — and a sub-topic with
                   no parent is worth calling out, because it shows under no subject
                   anywhere in the app. -->
              <td>
                <span v-if="item?.subject_name">{{ item.subject_name }}</span>
                <span v-else style="color:var(--amber);font-size:0.75rem;font-weight:600">
                  — no subject
                </span>
              </td>
