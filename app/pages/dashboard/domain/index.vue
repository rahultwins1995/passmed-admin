...
import EditDomainModal from '@/components/domains/EditDomainModal.vue';
...
const addDelete = async (id="0") => {
  ...
  try {
    const res:any = await $api.delete("/domains/delete/"+id)
    const obj:any = res.data
    if (obj.status === 'success') {
  ...
  <div class="section-hdr">
    <div class="section-hdr-left">
     ...
     <div class="section-hdr-right">
      <button class="btn btn-primary btn-sm" @click="openAddDomainModal">
        Add Domain
      </button>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="table-wrap">
        ...
        <table class="CategoryTable">
          ...
          <tbody v-if="data_loading || filteredDomain.length === 0">
            <tr>
              <td colspan="15">
          ...
          <tbody v-else>
            <tr v-for="(item, i) in filteredDomain" :key="i">
              <td>
                {{i+1}}
              </td>
              <td>{{ item.name }}</td>
