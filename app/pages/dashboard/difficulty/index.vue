...
import AddDifficultyModal from '@/components/difficulties/AddDifficultyModal.vue';
import EditDifficultyModal from '@/components/difficulties/EditDifficultyModal.vue';
...
const limit_data = ref(25)   // there are only a handful of levels — one page is plenty
...
const fetchData = async () => {
  ...
  try {
    const res: any = await $api.post("/difficulties", {
...
const confirmDelete = async (item: any) => {
  // Questions store the SLUG as a string, not a foreign key — deleting a level in
  // use leaves them holding a value that no longer exists in the vocabulary. They
  // keep rendering but silently fall out of every difficulty filter. The API refuses
  // it; say so here rather than making them find out.
...
const addDelete = async (id = "0") => {
  ...
  try {
    const res: any = await $api.delete("/difficulties/delete/" + id)
    const obj: any = res.data
    if (obj.status === 'success') {
...
<div class="dashwrap">
  ...
  <div style="font-size:0.76rem;color:var(--ink-dim);margin-bottom:12px;line-height:1.55">
    The levels a question can be rated at. Unlike the other taxonomies, questions store
    the <strong>slug</strong> of the level, not its id — so the slug is fixed once
    created and only the display <strong>Name</strong> can change. Order is by rank, not
    alphabetical.
  </div>
        ...
        <table class="DifficultyTable">
          <thead>
            <tr>
              <th>Order</th>
              <th>Name</th>
              <!-- Shown because it is what questions and import sheets actually carry. -->
              <th>Slug</th>
              <th>Questions</th>
              <th>Status</th>
              <th>Action</th>
          ...
          <tbody v-else>
            <tr v-for="(item, i) in getDataList" :key="i">
              ...
              <td>{{ item.name }}</td>
              <td><code style="font-size:0.78rem">{{ item.slug }}</code></td>
