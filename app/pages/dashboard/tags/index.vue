...
const fetchData = async () => {
  ...
  try {
    const res:any = await $api.post("/tags", {
...
const addDelete = async (id="0") => {
  ...
  try {
    const res:any = await $api.delete("/tags/delete/"+id)
    const obj:any = res.data
    if (obj.status === 'success') {
...
<div class="dashwrap">
  ...
  <div class="section-hdr">
    ...
    <div class="section-hdr-right">
      <button class="btn btn-primary btn-sm" @click="openAddTagsModal">
        Add Tag
      </button>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="table-wrap">
        ...
        <table class="tagsTable">
          ...
          <tbody v-if="data_loading || filteredTags.length === 0">
            <tr>
              <td colspan="15">
          ...
          <tbody v-else>
            <tr v-for="(item, i) in filteredTags" :key="i">
              <td>
                {{i+1}}
              </td>
              <td>{{ item.name }}</td>
