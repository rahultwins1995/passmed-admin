// Exam-type icon library for the admin icon picker. MIRROR of the public frontend's
// app/utils/examIcons.ts — keep the two in sync. `key` is stored on exams.icon and
// the frontend resolves it back to the same SVG. Broad, self-contained set (24×24
// stroke icons) so no icon ever needs to be uploaded/added in future.
...
/** SVG inner-markup for an icon key (stethoscope fallback for unknown keys). */
export function examIconPath(key: string | undefined | null): string {
  if (!key) return DEFAULT_ICON
  return ICONS[String(key).toLowerCase().trim()] || DEFAULT_ICON
}

/** Picker options — key is stored on exams.icon; keep in sync with the frontend. */
