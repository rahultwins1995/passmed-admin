...
const props = defineProps<{
  modelValue: Boolean
  detail: any | null
  portalType?: string   // home | student | institute
}>()
...
const submitSave = async () => {
  ...
  try {
    const res:any = await $api.post('/faqs/update/'+props.detail.id, { ...addform, portal_type: props.portalType || 'home' });
    ...
    if (res.data.status === 'success') {
      ...
      const message = err?.response?.data?.msg || 'Failed to saved.'
     $toast(message,'error');
...
const editLoadPage=()=>{
  ...
  if(detail){
    addform.question=detail?.question??'';
    addform.answer=detail?.answer??'';
    addform.status=detail?.status??'1';
    addform.type = detail?.type ?? ((props.portalType || 'home') === 'home' ? 'getting-started' : 'other');
  }
}
      ...
      <div class="drawer-body">
        <div class="topwrapadd">
            ...
            <select class="form-input form-select" v-model="addform.type">
              <template v-if="(portalType || 'home') === 'home'">
