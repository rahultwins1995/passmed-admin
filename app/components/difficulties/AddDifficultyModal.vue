...
const submitSave = async () => {
  ...
  try {
    const res: any = await $api.post('/difficulties/add', addform);

    if (res.data.status === 'success') {
      $toast("Difficulty is saved");
      emit('saved', true);
      resetForm();
      closeModal();
    } else {
  ...
  <div class="drawer" style="width:400px;max-width:96vw">
    ...
    <div class="drawer-body">
      <div class="topwrapadd">
          ...
          <!-- The slug is derived from the name and then FROZEN. Questions store the
               slug, so it can never be regenerated without orphaning them. Worth
               saying out loud at creation time — this is the one chance to get it
               right. -->
          <div style="font-size:0.7rem;color:var(--ink-dim);margin-top:5px;line-height:1.5">
            The stored value (slug) is derived from this name and <strong>cannot be
            changed later</strong> — questions carry it. You can rename the display
            label freely afterwards.
          </div>
          </div>
          ...
          <div style="font-size:0.7rem;color:var(--ink-dim);margin-top:5px;line-height:1.5">
            Rank, lowest first. Difficulty is the one taxonomy with a meaningful order —
            alphabetical would list Advanced before Foundation.
          </div>
          </div>
