...
// A Subject is the TOP of the taxonomy — it has no parent, so there is no Category
// picker here any more. The hierarchy runs the other way: a sub-topic (category)
// names its parent. This form used to ask for a `category_id`, which is what left
// the Category manager showing an empty parent column.
...
const submitSave = async () => {
  ...
  try {
    const res:any = await $api.post('/subjects/add', addform);

    if (res.data.status === 'success') {
        fullLoading.value = false;
         $toast("Subject is Saved");
          emit('saved', true);
          resetForm();
  ...
  <div class="drawer" style="width:400px;max-width:96vw">
    ...
    <div class="drawer-body">
      <div class="topwrapadd">
          ...
          <!-- No Category picker: a Subject is the top of the tree. Sub-topics point
               up at it from the Category manager, not the other way round. -->
