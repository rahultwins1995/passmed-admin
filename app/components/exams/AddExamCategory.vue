...
const submitSave = async () => {
  ...
  try {
    const res:any = await $api.post('/exams-categories/add', addform);

    if (res.data.status === 'success') {
        fullLoading.value = false;
         $toast("Exam Category is Saved");
          // Pass the created name up so callers can auto-select it (the /add
          // response doesn't return the id). Truthy → existing @saved handlers
          // that just refresh keep working.
          emit('saved', addform.name);
          resetForm();
